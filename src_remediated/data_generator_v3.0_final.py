...
class ExtendedDataGenerator:
        ...
        while curr_date <= sim_end:
            ...
            for idx in report_indices:
                ...
                batch_rows.append({
                    ...
                    # PHASE 2 FIX: Use correct schema column names
                    "original_interest_rate": float(interest_rates[idx]),
                    "original_installment_amount": float(scheduled_payments[idx])
                })
    ...
    def hydrate_dataframe(self, df, table_name):
        ...
        PHASE 2 FIX: Preserves existing calculated columns with valid data.
        Only adds defaults for truly missing columns.
        """
        ...
        # PHASE 2 FIX: Also identify columns that exist but are all null
        # We'll only hydrate those, not columns with valid data
        cols_needing_hydration = []
        for col in required_cols:
            if col not in current_cols:
                cols_needing_hydration.append(col)
            elif col in current_cols:
                # Check if column is all null or all zero (for numeric)
                col_data = df[col]
                if col_data.null_count() == len(df):
                    # All nulls - needs hydration
                    cols_needing_hydration.append(col)
                # If has valid data, skip hydration to preserve calculated values

        if cols_needing_hydration:
            print(f"    Adding {len(cols_needing_hydration)} missing/null columns...")
            print(f"    Preserving {len([c for c in current_cols if c not in cols_needing_hydration])} existing columns with data")
            new_exprs = []
            ...
            for col, dtype in zip(required_cols, data_types):
                # PHASE 2 FIX: Only process columns that need hydration
                if col not in cols_needing_hydration: continue
